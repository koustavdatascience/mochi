...
import {
  ...
  resolveCloneCrawl,
  type IonCapturedRenderer,
  type IonClonePlanV1,
  type IonRendererPlan,
} from "./plannedClone.js";
import { sha256Json } from "./contentManifest.js";
...
export type CloneSiteOptions = {
  ...
  motion?: boolean; // capture animation evidence (default true for direct legacy callers)
  ...
  experimentalClonePlan?: IonClonePlanV1; // private Ion-authored plan; requires the handoff opt-in
...
export function routeCaptureEvidenceOptions(
  opts: Pick<CloneSiteOptions, "interactions" | "motion">,
  full: boolean,
): { interactions: boolean; motion: boolean } {
  return {
    interactions: Boolean(opts.interactions && full),
    motion: Boolean((opts.motion ?? true) && full),
  };
}
...
export function plannedCloneFailedRoutes(params: {
  plan: IonClonePlanV1;
  plannedRenderers: IonCapturedRenderer[];
  capturedPaths: ReadonlySet<string>;
  cmsRoutes: ReadonlySet<string>;
  mappedManifestKeys: ReadonlySet<string>;
}): Array<{ routePath: string; reason: string }> {
  ...
  const resolveRoot = (renderer: IonRendererPlan): IonRendererPlan =>
    renderer.reuseRendererKey
      ...
      : renderer;
  const capturedRootKeys = new Set(
    params.plannedRenderers
      .filter((renderer) => params.capturedPaths.has(renderer.captureUrl))
      .map((renderer) => renderer.rendererKey),
  );
  const failures = params.plannedRenderers
    .filter((renderer) => !params.capturedPaths.has(renderer.captureUrl))
    .map((renderer) => ({
      routePath: renderer.captureUrl,
      reason: `renderer capture failed: ${renderer.rendererKey}`,
    }));

  for (const disposition of params.plan.dispositions) {
    if (
      disposition.kind !== "renderer" ||
      params.cmsRoutes.has(disposition.path) ||
      params.capturedPaths.has(disposition.path)
    ) {
      continue;
    }
    const renderer = rendererByKey.get(disposition.rendererKey!);
    if (!renderer) continue;
    const root = resolveRoot(renderer);
    if (!capturedRootKeys.has(root.key)) continue;
    const missingManifestKeys = renderer.manifestKeys.filter(
      (key) => !params.mappedManifestKeys.has(key),
    );
    if (missingManifestKeys.length > 0) {
      failures.push({
        routePath: disposition.path,
        reason: `structured content extraction unavailable for ${missingManifestKeys.join(", ")}`,
      });
    }
  }
  ...
  return failures.filter(
    (failure, index, all) =>
      all.findIndex(
        (candidate) => candidate.routePath === failure.routePath,
      ) === index,
  );
}

export async function runCloneSite(
  opts: CloneSiteOptions,
): Promise<CloneSiteResult> {
  const log = opts.log ?? (() => {});
  if (
    opts.experimentalContentHandoff === "ion-cms-v1" &&
    !opts.experimentalClonePlan
  ) {
    throw new Error(
      "experimentalContentHandoff requires experimentalClonePlan",
    );
  }
  const experimentalClonePlan =
    opts.experimentalContentHandoff === "ion-cms-v1"
      ? opts.experimentalClonePlan
      : undefined;
  ...
  const reuseEntrySource =
    (opts.reuseEntrySource &&
    fileExists(join(opts.reuseEntrySource, "capture", "capture-result.json"))
      ? opts.reuseEntrySource
      : null) ??
    (out && fileExists(join(runDir, "source", "capture", "capture-result.json"))
      ? join(runDir, "source")
      : null);
  if (experimentalClonePlan && !reuseEntrySource) {
    throw new Error(
      "planned ion-cms-v1 clones require a reusable entry capture",
    );
  }
  ...
  if (experimentalClonePlan) {
    log({
      event: "crawl_reused_from_plan",
      url: opts.url,
      paths: crawl.paths.length,
    });
  }
  const inferredPlan = selectRoutes({
    entryPath: crawl.entryPath,
    paths: crawl.paths,
    maxRoutes: opts.maxRoutes,
    maxCollectionInstances: opts.maxCollectionInstances,
  });
  let plannedRenderers: IonCapturedRenderer[] = [];
  let plan = inferredPlan;
  if (experimentalClonePlan) {
    const planned = buildIonPlannedRoutePlan(
      experimentalClonePlan,
      crawl,
      inferredPlan,
    );
    plan = planned.routePlan;
    plannedRenderers = planned.capturedRenderers;
  }
  writeJSON(join(runDir, "crawl.json"), {
    ...
    depthByPath: crawl.depthByPath,
    robotsDisallow: crawl.robotsDisallow,
  });
  log({
    event: "plan",
    discovered: crawl.paths.length,
  ...
  const captureRoute = async (
    ...
    const work = (async (): Promise<Built | null> => {
      ...
      try {
        ...
        const evidence = routeCaptureEvidenceOptions(opts, full);
        ...
        if (viewports.length === REQUIRED_VIEWPORTS.length)
          built.set(routePath, b);
        log({
          event: "route_captured",
          path: routePath,
          nodes: ir.doc.nodeCount,
          vps: viewports.length,
        });
        return b;
      } catch (e) {
        if (routePath === crawl.entryPath) throw e;
        log({
          event: "route_capture_failed",
          path: routePath,
  ...
  if (reuseEntrySource) {
    ...
    try {
      const capture = readJSON<CaptureResult>(
        join(entrySrc, "capture", "capture-result.json"),
      );
      const ir = buildIR(entrySrc, capture.viewports);
      const assetGraph = buildAssetGraph(capture);
      const fontGraph = buildFontGraph(
        capture.fontFaces,
        assetGraph,
        origin + crawl.entryPath,
      );
      built.set(crawl.entryPath, {
        capture,
        ir,
        assetGraph,
        fontGraph,
        tokens: extractTokens(ir),
        sourceDir: entrySrc,
      });
      log({
        event: "route_reused",
      ...
      log({ event: "route_reuse_failed", error: String(e).slice(0, 200) });
      if (experimentalClonePlan) {
        throw new Error(
          `planned entry capture reuse failed: ${String(e).slice(0, 300)}`,
        );
      }
    }
  }
  ...
  await mapLimit(plan.selected, captureConcurrency, (r) =>
    captureRoute(r.path, [...REQUIRED_VIEWPORTS]),
  );
  ...
  const collectionVerdicts = experimentalClonePlan
    ? []
    : await mapLimit(
        plan.collections,
        captureConcurrency,
        async (c): Promise<{ template: string; similar: boolean }> => {
          const rep = built.get(c.representative);
          if (!rep || !c.siblingProbe)
          ...
          if (!sib || sib.ir.doc.nodeCount < 12 || rep.ir.doc.nodeCount < 12) {
            log({
              event: "collection_confirm",
              template: c.template,
              similar: true,
              reason: "degenerate_sample",
            });
            return { template: c.template, similar: true };
          }
  ...
  await mapLimit(
    plan.selected.filter((r) => !built.has(r.path)),
    captureConcurrency,
    (r) => captureRoute(r.path, [...REQUIRED_VIEWPORTS]),
  );
  ...
  if (opts.experimentalContentHandoff === "ion-cms-v1") {
    const entrySourceDir = built.get(crawl.entryPath)?.sourceDir;
    const capturedHtmlByPath = Object.fromEntries(
      (experimentalClonePlan?.renderers ?? [])
        .flatMap((renderer) =>
          renderer.captureUrl ? [renderer.captureUrl] : [],
        )
        .filter((path, index, paths) => paths.indexOf(path) === index)
        .map((path) => {
          const sourceDir = built.get(path)?.sourceDir;
          return [
            path,
            sourceDir
              ? capturedStructuredHtmlFromSourceDir(sourceDir)
              : undefined,
          ] as const;
        })
        .filter(
    ...
    contentBundle = await buildContentHandoffBundle({
      sourceUrl: opts.url,
      crawl,
      plan,
      experimentalClonePlan,
      ...(entrySourceDir
        ? {
            shopifyStorefrontOrigins:
              shopifyStorefrontOriginsFromCapture(entrySourceDir),
          }
        : {}),
      ...(Object.keys(capturedHtmlByPath).length
        ? { capturedHtmlByPath }
        : {}),
      log,
    });
  ...
  const cmsRoutes = new Set(
    contentBundle?.families.flatMap((family) =>
      family.entries.map((entry) => entry.routePath),
    ) ?? [],
  );
  if (contentBundle && experimentalClonePlan) {
    const capturedPaths = new Set(
      plan.selected
        .filter((route) => built.has(route.path))
        .map((route) => route.path),
    );
    const capturedRenderers = plannedRenderers.filter((renderer) =>
      built.has(renderer.captureUrl),
    );
    const failedRoutes = plannedCloneFailedRoutes({
      plan: experimentalClonePlan,
      plannedRenderers,
      capturedPaths,
      cmsRoutes,
      mappedManifestKeys: new Set(
        contentBundle.families.flatMap((family) => family.manifestKeys ?? []),
      ),
    });
    contentBundle.plannedClone = {
      version: "ion-clone-plan-v1",
      plan: experimentalClonePlan,
      capturedRenderers,
      failedRoutes,
    };
    const { manifestHash: _manifestHash, ...bundlePayload } = contentBundle;
    contentBundle.manifestHash = sha256Json(bundlePayload);
  }
  ...
  if (experimentalClonePlan) {
    linkTargets.set(
      experimentalClonePlan.entryRoute,
      routeToSegment(experimentalClonePlan.entryRoute).href,
    );
    for (const route of experimentalClonePlan.staticRoutes)
      linkTargets.set(route, routeToSegment(route).href);
    const capturedPaths = new Set(
      plan.selected
        .filter((route) => built.has(route.path))
        .map((route) => route.path),
    );
    for (const disposition of experimentalClonePlan.dispositions) {
      const local =
        disposition.kind === "static" ||
        (disposition.kind === "renderer" &&
          (cmsRoutes.has(disposition.path) ||
            capturedPaths.has(disposition.path)));
      linkTargets.set(
        disposition.path,
        local
          ? routeToSegment(disposition.path).href
          : origin + disposition.path,
      );
    }
  }
  ...
  for (const r of plan.selected) {
    ...
    routes.push({
      routePath: r.path,
      ir: b.ir,
      assetGraph: b.assetGraph,
      fontGraph: b.fontGraph,
      tokens: b.tokens,
      sourceDir: b.sourceDir,
      capture: b.capture,
      interaction: b.capture.interaction,
    });
  }
  // M4: detect chrome shared across routes (header/footer) to hoist into the layout.
  const entryArtifact =
    routes.find((r) => r.routePath === crawl.entryPath) ?? routes[0];
  const chrome = entryArtifact
    ? detectSharedChrome(routes.map((r) => r.ir))
    : { headerCount: 0, footerCount: 0 };
  if (chrome.headerCount || chrome.footerCount)
    log({
      event: "shared_chrome",
      header: chrome.headerCount,
      footer: chrome.footerCount,
      sig: entryArtifact ? chromeSignatureId(entryArtifact.ir, chrome) : "",
    });
  const gen = generateSiteApp({
    appDir,
    routes,
    linkTargets,
    origin,
    entryRoutePath: crawl.entryPath,
    chrome,
  ...
  if (contentBundle)
    writeJSON(join(appDir, DITTO_CONTENT_BUNDLE_PATH), contentBundle);
  writeJSON(
    join(runDir, "generated", "extracted-components.json"),
    gen.extracted,
  );
  writeJSON(join(runDir, "generated", "seo.json"), gen.seoInventory);
  writeText(
    join(runDir, "generated", "seo.md"),
    seoInventoryToMarkdown(gen.seoInventory),
  );

  // 6. Site manifest (incl. the CMS-handoff collection map).
  writeJSON(join(runDir, "site-plan.json"), {
    entry: plan.entry,
    ...
    ...(experimentalClonePlan ? { experimentalClonePlan } : {}),
  });
  writeJSON(join(runDir, "site-manifest.json"), {
    sourceUrl: opts.url,
    origin,
    entry: crawl.entryPath,
    chrome,
    extractComponents: !!opts.components,
    humanizeMode: opts.humanizeMode ?? "tailwind",
    framework: opts.framework ?? "next",
    reflow: !!opts.reflow,
    seo: gen.seoInventory.metrics,
    routes: gen.routes.map((r) => ({
      ...r,
      role: plan.selected.find((s) => s.path === r.routePath)?.role ?? "page",
    })),
    ...
    skipped: plan.skipped,
    assetsCopied: gen.assetsCopied,
    assetsMissing: gen.assetsMissing,
    components: gen.components,
    ...(contentBundle
      ? {
          experimentalContentHandoff: "ion-cms-v1",
          contentBundle: DITTO_CONTENT_BUNDLE_PATH,
          ...(experimentalClonePlan ? { experimentalClonePlan } : {}),
        }
      : {}),
  });
  ...
  if (validate) {
    siteReport = await validateSite(runDir, {
      tier: opts.tier ?? "stage2",
  ...
  if (out) {
    ...
    log({ event: "exported", app: out.appDir });
  } else {
    stableAppDir = writeLatestPointer(runsDir, siteId, runDir);
  }
  return {
    runDir,
    appDir: out ? out.appDir : appDir,
    siteId,
    plan,
    routes,
    siteReport,
...
function latestRunFor(runsDir: string, url: string): string | null {
  ...
  const runs = readdirSync(siteDir)
    .filter((d) => /^\d/.test(d))
    .sort();
  for (let i = runs.length - 1; i >= 0; i--) {
    if (existsSync(join(siteDir, runs[i]!, "site-manifest.json")))
      return join(siteDir, runs[i]!);
  }
  return null;
}
...
export async function regenerateSite(
  ...
  for (const r of m.routes) {
    ...
    if (!fileExists(join(sourceDir, "capture", "capture-result.json")))
      continue;
    const capture = readJSON<CaptureResult>(
  ...
  const savedBundlePath =
    m.experimentalContentHandoff === "ion-cms-v1"
      ? join(appDir, m.contentBundle ?? DITTO_CONTENT_BUNDLE_PATH)
      : null;
  const savedBundle =
    savedBundlePath && fileExists(savedBundlePath)
      ? readJSON<DittoContentBundleV1>(savedBundlePath)
      : null;
  const cmsRoutes = new Set(
    savedBundle?.families.flatMap((family) =>
      family.entries.map((entry) => entry.routePath),
    ) ?? [],
  );
  for (const c of m.collections) {
    ...
    for (const inst of c.instances) {
      if (!linkTargets.has(inst))
        linkTargets.set(
          inst,
          cmsRoutes.has(inst) ? routeToSegment(inst).href : repHref,
        );
    }
  }
  const entryArtifact =
    routes.find((r) => r.routePath === m.entry) ?? routes[0];
  const chrome = entryArtifact
    ? detectSharedChrome(routes.map((r) => r.ir))
    : { headerCount: 0, footerCount: 0 };
  ...
  rmSync(join(appDir, "public", "assets", "cloned"), {
    recursive: true,
    force: true,
  });
  const gen = generateSiteApp({
    ...
    linkTargets,
    origin: m.origin,
    entryRoutePath: m.entry,
    chrome,
    components: m.extractComponents,
    humanizeMode: m.humanizeMode,
  ...
  writeJSON(join(runDir, "site-manifest.json"), {
    ...
    routes: gen.routes.map((r) => ({
      ...
      role:
        (
          m.routes.find((x) => x.routePath === r.routePath) as
...
function printResult(url: string, res: CloneSiteResult): void {
  ...
  console.log(
    `  generated ${res.routes.length} route page(s); ${plan.collections.length} collapsed collection(s)\n`,
  );
  for (const r of plan.selected) {
    const built = res.routes.find((x) => x.routePath === r.path);
    console.log(
      `    ${r.role.padEnd(14)} ${routeToSegment(r.path).href}${built ? "" : "   (capture failed)"}`,
    );
  }
  if (plan.collections.length) {
    ...
    for (const c of plan.collections)
      console.log(
        `    ${c.template}  (${c.instanceCount}) → ${c.representative}  ${c.confirmed ? "[confirmed]" : "[unconfirmed]"}`,
      );
  }
  ...
  if (sr) {
    ...
    for (const r of sr.routes)
      console.log(
        `    ${r.report.scorecard.total.toString().padStart(5)}  ${r.report.gates0to6Pass ? "✅" : "❌"} ${r.href}`,
      );
  }
  console.log("");
}
...
async function main(): Promise<void> {
  ...
  if (!url) {
    console.error(
      "usage: clone-site <url> [--styling=tailwind|css] [--framework=next|vite] [--out=<dir>] [--validate]",
    );
    process.exit(1);
  }
  ...
  const outDir = outArg
    ? outArg.includes("=")
      ...
      : "output"
    : undefined;
  const maxRoutes = args
    .find((a) => a.startsWith("--max-routes="))
    ?.split("=")[1];
  const maxCollection = args
    .find((a) => a.startsWith("--max-collection="))
    ?.split("=")[1];
  const maxDepth = args.find((a) => a.startsWith("--depth="))?.split("=")[1];
  const concurrency = args
    .find((a) => a.startsWith("--concurrency="))
    ?.split("=")[1];
  const validationConcurrency = args
    .find(
      (a) =>
        a.startsWith("--validate-concurrency=") ||
        a.startsWith("--validation-concurrency="),
    )
    ?.split("=")[1];
  const viewportConcurrency = args
    .find((a) => a.startsWith("--viewport-concurrency="))
    ?.split("=")[1];
  const runsArg = args.find((a) => a.startsWith("--runs="))?.split("=")[1];
  const tier = args.find((a) => a.startsWith("--tier="))?.split("=")[1];
  const validate =
    args.includes("--validate") && !args.includes("--no-validate");
  ...
  const stylingArg = args
    .find((a) => a.startsWith("--styling="))
    ?.split("=")[1];
  if (stylingArg && stylingArg !== "tailwind" && stylingArg !== "css") {
    console.error(
      `invalid --styling=${stylingArg}; expected "tailwind" or "css"`,
    );
    process.exit(1);
  }
  // Styling output defaults to Tailwind v4; --css is kept as a compatibility alias.
  const humanizeMode =
    (stylingArg as "tailwind" | "css" | undefined) ??
    (args.includes("--css") ? ("css" as const) : ("tailwind" as const));
  const frameworkArg =
    args.find((a) => a.startsWith("--framework="))?.split("=")[1] ??
    (args.includes("--vite") ? "vite" : undefined);
  if (frameworkArg && frameworkArg !== "next" && frameworkArg !== "vite") {
    console.error(
      `invalid --framework=${frameworkArg}; expected "next" or "vite"`,
    );
    process.exit(1);
  }
  ...
  const regenArg = args.find(
    (a) => a === "--regen" || a.startsWith("--regen="),
  );
  if (regenArg) {
    const explicit = regenArg.includes("=")
      ? resolve(regenArg.split("=")[1]!)
      : null;
    const runDir =
      explicit ??
      latestRunFor(runsDir ?? resolve(process.cwd(), "..", "runs"), url);
    if (!runDir) {
      console.error("no existing run to regenerate for " + url);
      process.exit(1);
    }
    const res = await regenerateSite(runDir, {
      ...
      validationConcurrency: validationConcurrency
        ? parseInt(validationConcurrency, 10)
        : undefined,
      viewportConcurrency: viewportConcurrency
        ? parseInt(viewportConcurrency, 10)
        : undefined,
    ...
    if (basename(runDir) === ".clone") {
      ...
      console.log(
        JSON.stringify({ event: "exported", app: deliverable, ...pub }),
      );
    }
    printResult(url, res);
  ...
  const res = await runCloneSite({
    ...
    maxCollectionInstances: maxCollection
      ? parseInt(maxCollection, 10)
      : undefined,
  ...
  printResult(url, res);
  console.log(
    JSON.stringify({ event: "done", runDir: res.runDir, app: res.appDir }),
  );
}

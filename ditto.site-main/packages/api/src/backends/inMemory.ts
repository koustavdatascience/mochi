...
export class InMemoryBackend implements Backend {
  ...
  private makeBase(): string {
    return (
      this.deps.makeTempBase ??
      (() => mkdtempSync(join(tmpdir(), "api-clone-")))
    )();
  }
  ...
  async submit(
    ...
    if (
      this.activeClones > 0 ||
      this.deps.store.list().some((j) => j.status === "running")
    ) {
      throw new Error(
        "BUSY: another clone is already running — wait for it to finish (use npm run dev:api:stable to avoid hot-reload killing Playwright)",
      );
    }
    let reuseEntrySource: string | undefined;
    if (options?.experimentalReuseCaptureJobId) {
      const prior = this.deps.store.get(options.experimentalReuseCaptureJobId);
      const inMemorySource = prior?.result ? join(prior.result.runDir, "source") : "";
      const validInMemorySource =
        prior?.status === "succeeded" &&
        prior.kind === "clone" &&
        normalizeUrl(prior.url) === normalizeUrl(url) &&
        existsSync(join(inMemorySource, "capture", "capture-result.json"))
          ? inMemorySource
          : undefined;
      const persistedSource = this.deps.captureCacheDir
        ? join(
            this.deps.captureCacheDir,
            "jobs",
            options.experimentalReuseCaptureJobId,
            "source",
          )
        : "";
      let validPersistedSource: string | undefined;
      if (persistedSource && existsSync(join(persistedSource, "capture", "capture-result.json"))) {
        try {
          const capture = JSON.parse(
            readFileSync(join(persistedSource, "capture", "capture-result.json"), "utf8"),
          ) as { sourceUrl?: unknown };
          if (
            typeof capture.sourceUrl === "string" &&
            normalizeUrl(capture.sourceUrl) === normalizeUrl(url)
          ) {
            validPersistedSource = persistedSource;
          }
        } catch {
          // Invalid persisted captures are treated as unavailable.
        }
      }
      const source = validInMemorySource ?? validPersistedSource;
      if (!source) {
        throw new Error(
          `requested entry capture is unavailable for job ${options.experimentalReuseCaptureJobId}`,
        );
      }
      reuseEntrySource = source;
    }
    const id = randomUUID();
  ...
  private async runInBackground(
    ...
    try {
      const result = await this.deps.runJob({
        url,
        jobId: id,
        options,
        runsDir: base,
        captureCacheDir: this.deps.captureCacheDir,
        reuseEntrySource,
        log,
      });
  ...
  async status(jobId: string): Promise<JobView | null> {
    ...
    if (rec.status === "succeeded" && rec.result)
      return {
        ...buildRestSummary(jobId, rec.result),
        verify: rec.result.verify,
      };
    return {
      jobId,
      url: rec.url,
  ...
  async result(jobId: string): Promise<ResultOutcome | null> {
    ...
    if (rec.status === "succeeded" && rec.result)
      return {
        ready: true,
        result: buildRestResult(jobId, rec.result, `/v1/clones/${jobId}/files`),
      };
    ...
    return Object.entries(rec.result.files).map(([path, f]) =>
      f.kind === "text"
        ...
        : {
            ...
            bytes: f.bytes,
            sha256: f.sha256,
            binaryUrl: async () => `/v1/clones/${jobId}/files/${path}`,
          },
    );
  }
  ...
  async bundle(
    ...
    const rec = this.deps.store.get(jobId);
    if (!rec?.result) return null;
    const entries = Object.entries(rec.result.files).map(([path, f]) => ({
      path,
      bytes: readFileSync(f.absPath),
    }));

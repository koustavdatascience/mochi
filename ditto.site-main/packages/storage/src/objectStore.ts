...
const ctFor = (p: string) =>
  CT[extname(p).toLowerCase()] ?? "application/octet-stream";
...
export class ObjectArtifactStore implements ArtifactStore {
  constructor(
    private blob: BlobClient,
    private opts?: { presignExpiresSeconds?: number },
  ) {}
  ...
  private entryCaptureKey(jobId: string): string {
    return `entry-captures/${jobId}.tgz`;
  }

  async putEntryCapture(jobId: string, sourceDir: string): Promise<void> {
    await this.blob.put(
      this.entryCaptureKey(jobId),
      packEntryCapture(sourceDir),
      "application/gzip",
    );
  }
  ...
  async restoreEntryCapture(
    ...
    const bytes = await this.blob.get(this.entryCaptureKey(jobId));
    if (!bytes) return false;
    unpackEntryCapture(bytes, destinationDir);
    return true;
  }
  ...
  async binaryUrl(jobId: string, path: string): Promise<string> {
    return this.blob.presign(this.key(jobId, path), {
      expiresSeconds: this.opts?.presignExpiresSeconds,
  ...
  async uploadBundle(
    ...
    return this.blob.presign(key, {
      expiresSeconds: this.opts?.presignExpiresSeconds,
      downloadName: `clone-${jobId}.${format}`,
      contentType: ct,
    });
  }
  ...
  async remove(jobId: string): Promise<void> {
    ...
    await this.blob.delete(this.entryCaptureKey(jobId));
  }
}
